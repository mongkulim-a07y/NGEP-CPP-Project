...
void promptDeleteCustomer(CustomerList &customerApp, LoginList &users)
{
    ...
    // Look up the matching login too - deleting only the customer profile
    // and leaving the login behind creates an orphaned account that can
    // still log in but has no profile to find.
    User *matchingLogin = users.findUserByCustomerId(id);
    ...
    if (matchingLogin != nullptr)
        cout << "Login account \"" << matchingLogin->username << "\" will ALSO be deleted.\n";
    cout << "-----------------------------------------------\n";
    ...
    if (matchingLogin != nullptr)
    {
        string username = matchingLogin->username; // copy before the object is freed by deleteUser
...
void runOwnerMenu(CustomerList &customerApp, CategoryList &category, OrderList &orderApp, ProductList &productApp, LoginList &users, const string &username)
{
    ...
    do
    {
        ...
        case 12:
            ...
            break;
        case 13:
            promptDeleteCustomer(customerApp, users);
            break;
        case 14:
